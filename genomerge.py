def merge(geno_name):
    ...
    if merge_proceed in ("y", "yes"):
        ...
        #Making sure they still have the 1000G vcf files. They should if they have just harmonized, but they might have deleted them or something.
        ...
        # If the logfile contains warnings, write a text file '1000G_MergeWarnings.txt' with the SNPs that threw warnings.
            ...
            if os.path.exists('1000G_Phase3-merge.missnp'):
               #Read in missnp file
                ...
                for i in range(0, len(chr_1000G_Phase3_names)):
                    os.system('plink --bfile ' + chr_1000G_Phase3_names[i] + ' --exclude 1000G_warnings_missnp.txt '
                                                                             '--geno 0.01 --make-bed --out '
                ...
                print("\u001b[36;1m Successfully merged 1000G, though you should double-check the log file to be sure \u001b[0m")

            else:  # If only merge warnings exist, exclude from 1000G completely
                #Use plink to exclude the merge warning snps.
                for i in range(0, len(chr_1000G_Phase3_names)):
                    os.system('plink --bfile ' + chr_1000G_Phase3_names[
                        i] + ' --exclude 1000G_MergeWarnings.txt --geno 0.01 --make-bed --out ' +
                              chr_1000G_Phase3_names[i])
                #Remove old plink files.
                os.system('rm *~')
                #Try merge again.
        ...
        # If only the missnps exist, remove them in 1000G.
            ...
            for i in range(0, len(chr_1000G_Phase3_names)):
                os.system('plink --bfile ' + chr_1000G_Phase3_names[i] + ' --exclude 1000G_Phase3-merge.missnp '
                                                                         '--geno 0.01 --make-bed --out '
                                                                            + chr_1000G_Phase3_names[i])
            #Remove old plink files
            os.system('rm *~')
            ...
            sys.exit("\u001b[36;1m Unable to merge 1000G chromosome files. You should try to merge them on your own.\u001b[0m")
...
###Merge of house data and 1000G####
        ...
        if os.path.exists(geno_name + '_1000G_MergeWarnings.txt'):
            if os.path.exists(geno_name + '_1000G-merge.missnp'): #and if there are triallelic snps that need to be flipped (missnp)
                # If merge warnings and missnps exist, exclude warning snps from both 1000G and house dataset
                os.system('plink --bfile 1000G_Phase3' + ' --exclude ' + geno_name + '_1000G_MergeWarnings.txt '
                                                                                     '--geno 0.01 --make-bed '
                                                                                     '--out 1000G_Phase3')
        ...
        #If only the missnps exist, flip them in the house dataset.
            ...
            os.system('plink --bfile ' + geno_name + '_HarmonizedTo1000G --flip '
                      + geno_name + '_1000G-merge.missnp --geno 0.01 --make-bed --out ' + geno_name + '_HarmonizedTo1000G')
            #Remove old plink files.
            os.system('rm *~')
            #Retry the merge.
            ...
            #If mergewarnings and missnips don't exist, then hopefully the merge happened successfully on the first try.
        ...
        if os.path.exists(geno_name + '_1000G_merge2.log'):
            ...
            if os.path.exists(geno_name + '_1000G_merge2_warnings.txt'):
                if os.path.exists(geno_name + '_1000G_merge2-merge.missnp'): #And if there are still triallelic snps
                    ...
                    warnings_missnp[1].dropna(how='any').to_csv(geno_name + '_1000G_merge2_warnings_missnp.txt', sep='\t',
                                                                header=False, index=False)
                    #Remove all of these snps from 1000G dataset.
                    os.system('plink --bfile 1000G_Phase3 --exclude ' + geno_name + '_1000G_merge2_warnings_missnp.txt '
                                                                                    '--geno 0.01 --make-bed '
                                                                                    '--out 1000G_Phase3')
            ...
            #If only the missnps still exist, remove them in both datasets.
                ...
                #Exclude from 1000G dataset.
                os.system('plink --bfile 1000G_Phase3 --exclude ' + geno_name + '_1000G_merge2-merge.missnp --geno 0.01 '
            ...
            #If the merge didn't happen, but not because of merge warnings or missnps.
                ...
                sys.exit("\u001b[36;1m House dataset and 1000G did not successfully merge 2nd time, though not because of "
        ...
        if os.path.exists(geno_name + '_1000G_merge3.log'):
            ...
            if os.path.exists(geno_name + '_1000G_merge3.bim'):
                print("\u001b[36;1m House dataset and 1000G merged correctly on 3rd try. Though you should double-check "
                      "the log file, I can't predict every error.\u001b[0m")
