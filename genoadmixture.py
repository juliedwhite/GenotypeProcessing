def prep(admix_name):
    ...
    os.system('plink --bfile ' + admix_name + ' --indep-pairwise 50 10 2 --out Admixture/' + admix_name)

    if relative_check in ('y', 'yes'):
        # If they have relatives in their sample, get a list of the filenames for each set of people.
        user_sets = input('\u001b[34;1m Please give me a comma separated list of your set list filenames (with '
        ...
        for i in range(0,len(set_list)):
            ...
            os.system('plink --bfile ' + admix_name + ' --keep ' + set_list[i] + ' --extract Admixture/'
                      + admix_name + '.prune.in --make-bed --out Admixture/' + admix_name + '_Set' + set_name + '_LDPruned')
    ...
    #If there's no relatives, we can do all of this on the full genotype file.
        ...
        os.system('plink --bfile ' + admix_name + ' --extract Admixture/'
                  + admix_name + '.prune.in --make-bed --out Admixture/' + admix_name + '_LDPruned')
        ...
        with open ('Admixture/' + admix_name + '_Admixture_k3to6.pbs', 'w') as file:
            file.write('#PBS -l walltime=150:00:00\n'
                       ...
                       'cd $PBS_O_WORKDIR\n'
                       '\n'
                       'for K in {3..6}; do ./admixture --cv ' + admix_name + '_LDPruned.bed $K | tee ' + admix_name + '_LDPruned.log${K}.out; done')
        ...
        with open('Admixture/' + admix_name + '_Admixture_k7to9.pbs', 'w') as file:
            file.write('#PBS -l walltime=150:00:00\n'
                       ...
                       'cd $PBS_O_WORKDIR\n'
                       '\n'
                       'for K in {7..9}; do ./admixture --cv ' + admix_name + '_LDPruned.bed $K | tee ' + admix_name + '_LDPruned.log${K}.out; done')

        #End and give directions.
        print("\u001b[36;1m Transfer " + admix_name + "_LDPruned bed/bim/fam files, " + admix_name + "_Admixture_k3to6.pbs, and "
              + admix_name + "_Admixture_7to9.pbs files to the cluster.\n"
              "Submit them using qsub " + admix_name + "Admixture_k3to6.pbs and qsub " + admix_name + "Admixture_k7to9.pbs\n"
              "When you get your results, you should evaluate them to see which makes sense given your study population "
              "and which has the lowest CV value\u001b[0m")
