def harmonize_with_1000g(geno_name):
    ...
    for i in range(0, len(vcf_file_names)):
        ...
        if i < 22:
            ...
            subprocess.call('rm ' + geno_name + '_chr23.*')
    ...
    for i in range(0, len(harmonized_geno_names)):
        ...
        if os.path.getsize(af_checked_names[i] + '.bim') > 0:
            subprocess.call('rm ' + final_snp_lists[i])
            subprocess.call('rm ' + harmonized_geno_names[i] + '.*')
    ...
    if os.path.getsize(geno_name + '_HarmonizedTo1000G.bim') > 0:
        for i in range(0, len(af_checked_names)):
            subprocess.call('rm ' + str(af_checked_names[i]) + '.*')
    ...
    if os.path.getsize(geno_name + '_HarmonizedTo1000G.reverse') > 0:
        subprocess.check_output('plink --bfile ' + geno_name + '_HarmonizedTo1000G --flip ' + geno_name
                                + '_HarmonizedTo1000G.reverse --make-bed --out ' + geno_name
                                + '_HarmonizedTo1000G_StrandChecked')
    # If .reverve doesn't exist, still make a new file to signify that you've done the strand check.
    else:
        subprocess.check_output('plink --bfile ' + geno_name + '_HarmonizedTo1000G --make-bed --out ' + geno_name
                                + '_HarmonizedTo1000G_StrandChecked')
