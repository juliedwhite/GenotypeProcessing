def harmonize_with_1000g(geno_name):
    ...
    for i in range(0, len(vcf_file_names)):
        ...
        if i < 22:
            subprocess.check_output('java -Xmx1g -jar "' + harmonizer_path + '/GenotypeHarmonizer.jar" $* '
                                                                     '--input ' + geno_name + 
            ...
            subprocess.check_output('plink --bfile ' + geno_name + ' --chr X --make-bed --out ' + geno_name + '_chr23')
    ...
    for i in range(0, len(harmonized_geno_names)):
        ...
        if os.path.getsize(af_checked_names[i] + '.bim') > 0:
            subprocess.call('rm ' + str(final_snps_by_chr[i]) + '.txt')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.bed')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.bim')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.fam')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.log')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.frq')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.nosex')
            subprocess.call('rm ' + str(harmonized_geno_names[i]) + '.hh')
    ...
    subprocess.check_output('plink --merge-list HouseMergeList.txt --geno 0.01 --make-bed --out ' + geno_name
                            + '_HarmonizedTo1000G')

    if os.path.getsize(geno_name + '_HarmonizedTo1000G.bim') > 0:
        for i in range(0, len(af_checked_names)):
            subprocess.call('rm ' + str(af_checked_names[i]) + '.bed')
            subprocess.call('rm ' + str(af_checked_names[i]) + '.bim')
            subprocess.call('rm ' + str(af_checked_names[i]) + '.fam')
            subprocess.call('rm ' + str(af_checked_names[i]) + '.log')
            subprocess.call('rm ' + str(af_checked_names[i]) + '.nosex')
            subprocess.call('rm ' + str(af_checked_names[i])+ '.hh')
    else:
    ...
    try:
        subprocess.check_output('snpflip --fasta-genome = ' + os.path.join(fasta_path, 'human_g1k_v37.fasta.gz')
                                + ' --bim-file = ' + geno_name + '_HarmonizedTo1000G --output-prefix = ' + geno_name
                                + '_HarmonizedTo1000G')
    except ImportError:
    ...
    # If SNPs exist that have ambiguous strand, then remove them
        ...
        subprocess.check_output('plink --bfile ' + geno_name + '_HarmonizedTo1000G --exclude '
                                + geno_name + '_HarmonizedTo1000G.ambiguous --make-bed --out '
                                + geno_name + '_HarmonizedTo1000G_StrandChecked')
