def ibd(geno_name):
    ...
    #Perform IBD calculation, filtering for a minimum of 0.1875. This is the halfway point between 2nd and 3rd degree relatives.
    os.system('plink --bfile ' + geno_name + ' --exclude IBS_Calculations/' + geno_name + '.prune.out --genome --min 0.1875 --out IBD_Calculations/' + geno_name)
    #Change separator of IBD file from space to tab separated.
    os.system('sed -r "s/\s+/\t/g" IBD_Calculations/' + geno_name + '.genome > IBD_Calculations/' + geno_name + '.tab.genome')
